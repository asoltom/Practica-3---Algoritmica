...
public class Controller : MonoBehaviour
{
    ...
    private void BFS(int currIndex, List<int> copIndices)
    {
        ...
        Queue<Tile> vertices = new Queue<Tile>();

        // Añadimos a la cola todos los vertices adyacentes a la casilla actual
        foreach (int i in tiles[currIndex].adjacency)
        {
            tiles[i].parent = tiles[currIndex];  // Casilla raíz
            vertices.Enqueue(tiles[i]);
        }

        // Mientras haya vertices en la cola
        while (vertices.Count > 0)
        {
            Tile curr = vertices.Dequeue(); //curr ==> current ==> actual
            if (!curr.visited)
            {
                ...
                {
                    // Si la casilla actual no contiene un policía, añadimos a la cola todos sus vertices adyacentes
